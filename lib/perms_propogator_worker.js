   ...
   async.series(funs, function (err, results) {
      ...
      if (err) {
         ...
         dbs['permissions'].get(msg.database, function (err, db_body) {
            ...
            if (err && 13 === err.code){
               logger.info("Permissions do not exist: update failed");
            }
            else{
               ...
               dbs['permissions'].replace(msg.database, db_body.value, function (err, result) {
                  ...
                  logger.info("Permissions have been propogated");
               })
            }
         });
      }
...
var init = function(config, log){

   logger                 = log
   var cluster            = new couchbase.Cluster( config.couchbase_cluster );
