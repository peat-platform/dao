...
var checkPermissions = function(msg, callback, success_function){
   ...
   if (openiUtils.isTypeId(msg.database)){
      success_function()
   }
   else{
...
var getAction = function (msg, callback) {
   ...
   db_use.get(msg.database, options, function (err, db_body) {
      ...
      if (err) {
         ...
         if (openiUtils.isTypeId(msg.database) || openiUtils.isCloudletId(msg.database) || "permissions" === msg.resp_type){
            processGetData(db_body, msg, callback);
         }
         else if ( undefined === db_body.value._permissions[msg.third_party] || !db_body.value._permissions[msg.third_party].read ){
            callback(null, { 'error': 'Permission denied' }, HTTPStatus.UNAUTHORIZED);
         }
         else{
            processGetData(db_body, msg, callback);
