...
var getDbLazy = function (name) {
   ...
   switch (name) {
      ...
      // case "permissions":
      // case "objects":
      case "users": /*guard against create atk, should be impossible via API thou*/
      case "clients":
      case "authorizations":
         if(dbs[name] == undefined);
            dbs[name] = cluster.openBucket(name); 
   }

   return dbs[name];
};

var genericAction = function (msg, action, callback) {

   var options = (undefined === msg.format) ? {} : {format: msg.format };
   var db_use = getDbLazy(msg['bucket']);
   var status = HTTPStatus.INTERNAL_SERVER_ERROR;

   switch(action) {
      case 'CREATE':
         action = db_use.insert;
         status = HTTPStatus.CREATED;
      case 'READ':
         action = db_use.get;
         status = HTTPStatus.OK;
      case 'UPDATE':
         action = db_use.upsert;
         status = HTTPStatus.OK;
      case 'DELETE':
         action = db_use.remove;
         status = HTTPStatus.OK;
      default:
         action = undefined;
   }
   ...
   if(action) {
      action(msg.database, msg.object_data, options, function (err, result) {

         if (err) {
            status = (12 === err.code) ? HTTPStatus.CONFLICT : HTTPStatus.NOT_FOUND;
            callback(null, { 'error': 'Error adding entity: ' + getCouchbaseError(err, msg.database, msg.id) }, httpCode);
         }
         else {
            callback(null, { '@id': msg.id, 'data': result }, status);
         }
      });
   }
};
...
var evaluateAction = function (msg, callback) {
   ...
   dbc.hasMemberIn(msg, 'action', ['POST', 'PUT', 'GET', 'FETCH', 'CREATE', 'DELETE', 'VIEW', 'GET_OR_POST', 'PATCH_ATTACHMENT', 'PATCH_ATTACHMENT_OBJECT', 'UPDATE_PERMISSIONS', 'GENERIC_CREATE', 'GENERIC_READ', 'GENERIC_UPDATE', 'GENERIC_DELETE']);
   dbc.conditionalHasMember(msg, 'object_data', (msg.action === 'POST' || msg.action === 'PUT' || msg.action === 'GENERIC_CREATE' || msg.action === 'GENERIC_UPDATE'));
   ...
   switch (msg.action) {
      case 'GENERIC_CREATE':
         resp_msg = genericAction(msg, 'CREATE', callback);
         break;
      case 'GENERIC_READ':
         resp_msg = genericAction(msg, 'READ', callback);
         break;
      case 'GENERIC_UPDATE':
         resp_msg = genericAction(msg, 'UPDATE', callback);
         break;
      case 'GENERIC_DELETE':
         resp_msg = genericAction(msg, 'DELETE', callback);
         break;
      case 'POST':
