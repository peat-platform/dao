...
var postAction = function (msg, callback) {
   ...
   db_use.add(msg.database, msg.object_data, options, function (err, result) {
      if (err) {
         ...
         if(err.code === 12 && 0 === msg.database.indexOf('t_'))
         {
...
var recurseReplaceSubRef = function (sub, subs) {
   ...
   for (var i in sub['@data']) {
      if (sub['@data'].hasOwnProperty(i)) {
         var val = sub['@data'][i];
         if (typeof val == 'string' && -1 !== val.indexOf('o_')) {
            for (var j in subs) {
...
var isObjectId = function(str){
   ...
   if ( 0 !== str.indexOf('o_') || -1 === str.indexOf('-')){
      return false
   }
   str = str.substring(0, str.indexOf('-'))

   return (str.length === 34) ? true : false;
}
...
// TODO or NOT TODO... that is the question: sort out replication of these two loops
// Two loops are required due to the recursive calls
