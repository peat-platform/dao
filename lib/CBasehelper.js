...
var postAction = function (msg, callback) {
   ...
   db_use.add(msg.database, msg.object_data, options, function (err, result) {
      if (err) {
         ...
         console.log("1")
         console.log(err)
         console.log(getCouchbaseError(err, msg.database, msg.rest_uuid))
         console.log("2")
         callback(null,
...
var typeHelper = function(db_body, msg){
   ...
   if (undefined === db_body.value._id){
      db_body.value = { '_id' : db_body.value }
   }
...
var objectHelper = function(db_object, msg){
   ...
   if (undefined === db_object.value._id){
      resp_obj = { '_id' : db_object.value }
   }
   ...
   db_use.get(msg.database, function (err, db_body) {
      ...
      if (err) {
         ...
         if (0 === msg.database.indexOf('t_')){
            var resp = typeHelper(db_body, msg)
         }
         else if(msg['resp_type'] === 'cloudlet'){
            var resp = cloudletHelper(db_body, msg)
         }
         else if(msg['resp_type'] === 'binary'){
            headers  = {'Content-Type': db_body.value["Content-Type"]}
            resp = new Buffer(db_body.value['file'], 'base64')
         }
         else if(msg['resp_type'] === 'binary_meta'){
            db_body.value['file'] = undefined
            resp = db_body.value
         }
         else{
            var resp = objectHelper(db_body, msg)
         }
