         ...
         if(err || (data.response.meta === undefined || data.response.meta.length === 0)) {
            ...
            for (var i = 0; i < data.response.result.length; i++){
               ...
               if (api_key === perms_api_key){
                  ...
                  for ( var j in perms['@service_enabler']){
                     var se_ent = perms['@service_enabler'][j]

                     var se_cloudlet = se_ent['app_id']
                     var se_api_key  = se_ent['cloudlet']

                     permissions[se_cloudlet] = {
                        "create": create,
                        "read"  : read,
                        "update": update,
                        "delete": delet
