...
var init = function (log, tracklet_worker, perms_prop_f_conf) {
   ...
   dbs['objects'].enableN1ql('localhost:8093');
...
var genericView = function (msg, callback) {
   ...
   var query = ViewQuery.from(msg.design_doc, msg.view_name)
      ...
      .range(msg.start_key, msg.start_key);
...
var putAction = function (msg, callback) {
   ...
   /* istanbul ignore next: Unable to test as CouchbaseMock and Couchbase CAS have different functionality.
   * The current revision verification is not supported by CouchabseMock CAS
   * (db_body.cas['1'] is undefined)
   */
...
var viewAction = function (msg, callback) {
   ...
   var query = ViewQuery.from(msg.design_doc, msg.view_name)
      ...
      .limit(msg.meta.limit)
      .stale(ViewQuery.Update.BEFORE)
      .range(msg.start_key, msg.end_key);
...
var evaluateMessage = function (msg, myCallback) {
   ...
   async.series(arr, function (err, results) {
      ...
      var httpStatusCode = results['0'][1];

      myCallback(err, data, httpStatusCode);
   });
