      ...
      db_use.get(msg.bucket, function (err, result) {
         if (err || result.value.key !== key) {
            db_use.get(key, function (err, result) {
               if (err || Array.isArray(result.value.dbs) && result.value.dbs.indexOf(key) > -1) {
...
var genericCreate = function (msg, callback) {
   ...
   var db_use = getDbLazy(msg['bucket'], function(err)
   {
      ...
      db_use.insert(msg.id, msg.object_data, msg.options, function (err, result) {
         if (err) {
            ...
            callback(null, { 'action': 'create', 'db': msg['bucket'], 'id': msg.id, 'data': msg.object_data }, HTTPStatus.CREATED);
         }
      });
   ...
   var db_use = getDbLazy(msg['bucket'], function(err)
   {
      ...
      db_use.get(msg.id, msg.options, function (err, result) {
         if (err) {
            ...
            callback(null, { 'action': 'read', 'db': msg['bucket'], 'id': msg.id, 'data': result.value }, HTTPStatus.OK);
         }
      });
...
var genericUpdate = function (msg, callback) {
   ...
   var db_use = getDbLazy(msg['bucket'], function(err)
   {
      ...
      db_use.get(msg.id, msg.options, function (err, result_old) {
         ...
         db_use.upsert(msg.id, msg.object_data, msg.options, function (err, result) {
            if (err) {
               callback(null, { 'error': 'Error update entity: ' + getCouchbaseError(err, msg.database, msg.id) }, HTTPStatus.BAD_REQUEST);
            }
            else {
               callback(null, { 'action': 'update', 'db': msg['bucket'], 'id': msg.id, 'data': result_old.value }, HTTPStatus.OK);
            }
         });
      });
      ...
      db_use.get(msg.id, msg.options, function (err, result2) {
         ...
         db_use.remove(msg.id, msg.options, function (err, result) {
            if (err) {
               ...
               callback(null, { 'action': 'delete', 'db': msg['bucket'], 'id': msg.id, 'data': result2.value }, HTTPStatus.OK);
            }
         });
      });
      ...
      db_use.get(msg.id, msg.options, function (err, result) {
         ...
         db_use.upsert(msg.id, o, msg.options, function (err, result) {
            if (err) {
               ...
               callback(null, { 'action': 'patch', 'db': msg['bucket'], 'id': msg.id, 'data': o }, HTTPStatus.OK);
            }
         });
      });
