...
var checkPermissions = function(msg, callback, success_function){
   ...
   if (openiUtils.isTypeId(msg.database) || openiUtils.isAttachmentId(openiUtils.extractAttachmentId(msg.database))){
      ...
      ///needs to be turned into a loop through all of this cloudlets permissions....
      //get all permissions
      ...
      genericView(list_permissions_for_cloudlet_msg, function(err, data, httpcode){
         ...
         if(err || data.response.meta.length === 0) {
            loglet.error(err);
            ...
            for (var i = 0; i < data.response.result.length; i++){
               ...
               var entry             = data.response.result[i].perms

               if (perms_third_party === third_party || isServiceEnabler(perms_third_party, entry._current.perms['@service_enabler'])){

                  if (perms_third_party === third_party ){
                     if (undefined === entry._current.perms['@types'][type_id]){
                        callback(null, { 'error': 'permission denied' }, HTTPStatus.UNAUTHORIZED);
                        return
                     }
                  ...
                  var create_a = entry._current.perms['@types'][type_id]['@app_level'].create
                  var create_b = entry._current.perms['@types'][type_id]['@cloudlet_level'].create
                  var read_a   = entry._current.perms['@types'][type_id]['@app_level'].read
                  var read_b   = entry._current.perms['@types'][type_id]['@cloudlet_level'].read
                  var update_a = entry._current.perms['@types'][type_id]['@app_level'].update
                  var update_b = entry._current.perms['@types'][type_id]['@cloudlet_level'].update
                  var delete_a = entry._current.perms['@types'][type_id]['@app_level'].delete
                  var delete_b = entry._current.perms['@types'][type_id]['@cloudlet_level'].delete
                  ...
                  if (perms_third_party === third_party && !create){
                     callback(null, { 'error': 'permission denied' }, HTTPStatus.UNAUTHORIZED);
                     return
                  }
                  ...
                  var create = entry._current.perms['@types'][type_id]['@cloudlet_level'].create
                  var read   = entry._current.perms['@types'][type_id]['@cloudlet_level'].read
                  var update = entry._current.perms['@types'][type_id]['@cloudlet_level'].update
                  var delet  = entry._current.perms['@types'][type_id]['@cloudlet_level'].delete
...
var putAction = function (msg, callback) {
   ...
   db_use.get(msg.database, function (err, db_body) {
      ...
      if (db_body.value['@type'] === msg.object_data['@type'] && db_body.cas['1'].toString() !== revisionParts[1]) {
         callback(null, { 'error': 'Entity already updated'}, HTTPStatus.CONFLICT);
         return;
      }
...
var checkPermissions = function(msg, db_body, action){
   ...
   if (openiUtils.isTypeId(msg.database) || openiUtils.isCloudletId(msg.database) ){
      return true;
   }
   ...
   if ('objects' === msg.bucket){
      ...
      if (undefined === db_body.value._permissions || undefined === db_body.value._permissions[msg.third_party]){
         return false;
      }
      else{
         return db_body.value._permissions[msg.third_party][action]
      }
   }
...
var getAction = function (msg, callback) {
   ...
   db_use.get(msg.database, options, function (err, db_body) {
      ...
      if (err) {
         ...
         if (checkPermissions(msg, db_body, "read")){
      ...
      if (err) {
         ...
         for (var i in db_body){
            if (msg.resp_type === 'type'){
               ...
               if (checkPermissions(msg, db_body[i], "read")){
      ...
      db_use.query(query, function (err, res) {
         ...
         if (err) {
            ...
            if (msg.resp_type === 'type') {
               if (msg.id_only) {
                  for (var i = 0; i < res.length; i++) {
                     respObj.result[i] = openiUtils.extractTypeId(res[i]['id']);
                  }
               }
               else {
...
var deleteDBAction = function (msg, callback) {
   ...
   getDb('objects').get(msg.database, function (err, db_body) {
      ...
      else if (checkPermissions(msg, db_body, 'delete')){
...
var evaluateAction = function (msg, callback) {
   ...
   switch (msg.action) {
      ...
      case 'UPDATE_PERMISSIONS':
         ...
         break;
      case 'APP_PERMISSIONS':
         resp_msg = permissionsHelper.processAppPermissionsUpdate(couchbase, dbs, msg, callback)
         break;
