...
var init = function (logger_params) {
   ...
   dbs['openi'] = new couchbase.Connection({host: 'localhost:8091', bucket: 'openi', connectionTimeout:60000}, function (err) {
      if (err) {
         console.log('Connection Error', err);
         loglet.error(err, 'openi bucket connection error');
      }
      else {
         console.log('DAO: Connected openi bucket!');
         loglet.info('openi bucket connected');
      }
   });

   dbs['attachments'] = new couchbase.Connection({host: 'localhost:8091', bucket: 'attachments', connectionTimeout:60000}, function (err) {
      if (err) {
         console.log('Connection Error', err);
         loglet.error(err, 'attachments bucket connection error');
      }
      else {
         console.log('DAO: Connected attachments bucket!');
         loglet.info('attachments bucket connected');
      }
   });
