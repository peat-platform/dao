      ...
      if (err) {
         ...
         for (var i in db_body){
            if (msg.resp_type === 'type'){
               ...
               if ( undefined === db_body[i].value._permission || undefined === db_body[i].value._permissions[msg.third_party] || db_body[i].value._permissions[msg.third_party].read ){
...
var deleteDBAction = function (msg, callback) {
   ...
   getDb('objects').get(msg.database, function (err, db_body) {
      ...
      else if ( undefined === db_body.value._permissions[msg.third_party] ||
            !db_body.value._permissions[msg.third_party].delete ||
            !db_body.value['@cloudlet'] === msg.third_party){
         callback(null, { 'error': 'Permission denied' }, HTTPStatus.UNAUTHORIZED);
      }
      else{
